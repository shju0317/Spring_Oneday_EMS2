...
public class HomeController {
	...
	public String write(Model model) {
		...
		String cd = DateTimeFormatter.ofPattern("yyyy-MM-dd").format(ldt); // current date
		String ct = DateTimeFormatter.ofPattern("HH:mm:ss").format(ldt); // current time
...
//		model.addAttribute("s_subject", emsVO.getS_subject());
	 ...
	 * 해서 EmsVO를 DB로부터 Select하고 그 결과를 model에 담아서 다시 ems-write.jsp로 보내기
	 */
	...
	public String update(EmsVO emsVO, @RequestParam(value = "file1", required = false) MultipartFile file1,
		...
		// 3-6. upload된 파일이 없으면	
		} else if (file1_ex) { // 업로드된 파일이 없을 때
		...
		} else if (file2_ex) { // 업로드된 파일이 없을 때
